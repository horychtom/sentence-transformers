...
with gzip.open(sts_dataset_path, "rt", encoding="utf8") as fIn:
    ...
    for row in reader:
        ...
        if row["split"] == "dev":
            ...
            gold_samples.append(
                InputExample(
                    texts=[row["sentence2"], row["sentence1"]],
                    label=score,
                ),
            )
...
logging.info(
    "Step 2: Label QQP (target dataset) with cross-encoder: {}".format(
        model_name,
    ),
)
...
with open(
    os.path.join(qqp_dataset_path, "classification/train_pairs.tsv"),
    encoding="utf8",
) as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
...
with open(
    os.path.join(qqp_dataset_path, "classification/dev_pairs.tsv"),
    encoding="utf8",
) as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
...
with open(
    os.path.join(qqp_dataset_path, "classification/test_pairs.tsv"),
    encoding="utf8",
) as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
