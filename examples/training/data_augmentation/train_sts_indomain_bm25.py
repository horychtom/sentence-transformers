...
logging.info(
    "Indexing complete for {} unique sentences".format(len(unique_sentences)),
)
...
for sent, idx in sent2idx.items():
    res = es.search(
        index=index_name,
        body={"query": {"match": {"sent": sent}}},
        size=top_k,
    )
    progress.update(1)
    for hit in res["hits"]["hits"]:
        if idx != int(hit["_id"]) and (idx, int(hit["_id"])) not in set(
            duplicates,
        ):
...
logging.info(
    "Number of silver pairs generated for STSbenchmark: {}".format(
        len(silver_data),
    ),
)
logging.info(
