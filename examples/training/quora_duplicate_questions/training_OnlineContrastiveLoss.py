...
model_save_path = (
    "output/training_OnlineConstrativeLoss-"
...
dev_labels = []
with open(
    os.path.join(dataset_path, "classification/dev_pairs.tsv"),
    encoding="utf8",
) as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
...
dev_duplicates = []
with open(
    os.path.join(dataset_path, "duplicate-mining/dev_corpus.tsv"),
    encoding="utf8",
) as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
...
with open(
    os.path.join(dataset_path, "duplicate-mining/dev_duplicates.tsv"),
    encoding="utf8",
) as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
...
with open(
    os.path.join(dataset_path, "information-retrieval/dev-queries.tsv"),
    encoding="utf8",
) as fIn:
...
with open(
    os.path.join(dataset_path, "information-retrieval/corpus.tsv"),
    encoding="utf8",
) as fIn:
    next(fIn)  # Skip header
    for line in fIn:
