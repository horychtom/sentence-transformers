...
for row in rows:
    if row["qid1"] in is_assigned and row["qid2"] in is_assigned:
        ...
        if (
            row["qid2"] in is_assigned
        ):  # Ensure that qid1 is assigned and qid2 not yet
...
with open(
    "quora-IR-dataset/graph/sentences.tsv",
    "w",
    encoding="utf8",
) as fOut:
...
with open(
    "quora-IR-dataset/graph/duplicates-graph-pairwise.tsv",
    "w",
    encoding="utf8",
) as fOut:
...
with open(
    "quora-IR-dataset/graph/duplicates-graph-list.tsv",
    "w",
    encoding="utf8",
) as fOut:
    fOut.write("qid1\tqid2\n")
...
with open(
    "quora-IR-dataset/graph/duplicates-graph-connected-nodes.tsv",
    "w",
    encoding="utf8",
) as fOut:
    written_qids = set()
...
def write_qids(name, ids_list):
    with open(
        "quora-IR-dataset/graph/" + name + "-questions.tsv",
        "w",
        encoding="utf8",
    ) as fOut:
        fOut.write("qid\n")
...
with open(
    "quora-IR-dataset/classification/train_pairs.tsv",
    "w",
    encoding="utf8",
) as fOutTrain, open(
    "quora-IR-dataset/classification/dev_pairs.tsv",
    "w",
    encoding="utf8",
) as fOutDev, open(
    "quora-IR-dataset/classification/test_pairs.tsv",
    "w",
    encoding="utf8",
) as fOutTest:
    fOutTrain.write(
...
with open(
    "quora-IR-dataset/information-retrieval/corpus.tsv",
    "w",
    encoding="utf8",
) as fOut:
    fOut.write("qid\tquestion\n")
...
with open(
    "quora-IR-dataset/information-retrieval/dev-queries.tsv",
    "w",
    encoding="utf8",
) as fOut:
...
with open(
    "quora-IR-dataset/information-retrieval/test-queries.tsv",
    "w",
    encoding="utf8",
) as fOut:
    fOut.write("qid\tquestion\tduplicate_qids\n")
