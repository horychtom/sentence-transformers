...
queries = (
    {}
)  # dict in the format: query_id -> query. Stores all training queries
...
with gzip.open(hard_negatives_filepath, "rt") as fIn:
    for line in tqdm.tqdm(fIn):
        ...
        if negs_to_use is None:
            if (
                args.negs_to_use is not None
            ):  # Use specific system for negatives
                negs_to_use = args.negs_to_use.split(",")
            else:  # Use all systems
                negs_to_use = list(data["neg"].keys())
            logging.info(
                "Using negatives from the following systems:",
                negs_to_use,
            )
