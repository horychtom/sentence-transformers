...
source_languages = set(
    ["en"],
)  # Languages our (monolingual) teacher model understands
...
for source_lang in source_languages:
    for target_lang in target_languages:
        ...
        if not os.path.exists(filename_train) and not os.path.exists(
            filename_dev,
        ):
            langs_ordered = sorted([source_lang, target_lang])
            wikimatrix_filename = "WikiMatrix.{}-{}.tsv.gz".format(
                *langs_ordered,
            )
            wikimatrix_filepath = os.path.join(
                download_folder,
                wikimatrix_filename,
            )
            ...
            if not os.path.exists(wikimatrix_filepath):
                ...
                try:
                    sentence_transformers.util.http_get(
                        download_url + wikimatrix_filename,
                        wikimatrix_filepath,
                    )
                except:
                    print(
            ...
            with gzip.open(wikimatrix_filepath, "rt", encoding="utf8") as fIn:
                for line in fIn:
                    ...
                    if (
                        sent1 in dev_sentences_set
                        or sent2 in dev_sentences_set
                    ):
                        continue
