...
from sentence_transformers import evaluation, losses, models, util
import logging
import os
import gzip
...
dev_test_ids = set()
with gzip.open(
    os.path.join(askubuntu_folder, "text_tokenized.txt.gz"),
    "rt",
    encoding="utf8",
) as fIn:
    for line in fIn:
...
dev_evaluator = evaluation.RerankingEvaluator(
    dev_dataset,
    name="AskUbuntu dev",
)
test_evaluator = evaluation.RerankingEvaluator(
    test_dataset,
    name="AskUbuntu test",
)
